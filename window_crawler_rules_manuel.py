...
class system_control:
    ...
    def update_goal(self, rout, path, end_point, craw_loc, craw,
        ...
        global drop_route, reset_simu_global, tmp, bx, by
        poss = [split(craw_loc,0),split(craw_loc,1)]
        if tmp is "a":
            poss[1] -= 1
            print(poss)
            if poss[1] < 0:
            ...
            poss = poss
            #print(tmp)
        if (str(poss[0]) + "," + str(poss[1])) == craw_loc:
            self.modify(craw, goal="None")
        else:
            self.modify(craw, goal=(str(poss[0]) + "," + str(poss[1])))
        tmp = ""
        '''
        if drop_route:
...
class control_crawler (threading.Thread):
    def __init__(self):
        threading.Thread.__init__(self)
